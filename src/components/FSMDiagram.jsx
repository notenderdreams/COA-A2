...
export const FSMDiagram = memo(function FSMDiagram({ active, fillPct, activeEdge }) {
  const edges = [
    { f: 'Idle', t: 'Compare_Tag', lbl: 'Valid CPU request', cp: null },
    { f: 'Compare_Tag', t: 'Idle', lbl: 'Cache Hit / Mark Ready', cp: { x: 200, y: 30 } },
    { f: 'Compare_Tag', t: 'Write_Back', lbl: 'Miss + Dirty', cp: null },
    { f: 'Write_Back', t: 'Allocate', lbl: 'Memory Ready', cp: { x: 200, y: 330 } },
    { f: 'Compare_Tag', t: 'Allocate', lbl: 'Miss + Clean', cp: null, lblOff: { x: -20, y: 0 } },
    { f: 'Allocate', t: 'Compare_Tag', lbl: 'Memory Ready', cp: { x: 165, y: 185 } },
    { f: 'Write_Back', t: 'Write_Back', lbl: 'Mem not Ready', self: true, side: 'right' },
    { f: 'Allocate', t: 'Allocate', lbl: 'Mem not Ready', self: true, side: 'left' },
  ];
  ...
  function edgePath(e) {
    ...
    const p1 = nodePt(e.f, e.t), p2 = nodePt(e.t, e.f);
    if (e.cp) {
      const cpx = e.cp.x, cpy = e.cp.y;
      const isAlloc2Cmp = e.f === 'Allocate' && e.t === 'Compare_Tag';
      const lmx = isAlloc2Cmp ? cpx - 18 : cpx;
      const lmy = cpy + (e.f === 'Compare_Tag' && e.t === 'Idle' ? -10 : e.f === 'Write_Back' ? 14 : isAlloc2Cmp ? -8 : 0);
      return { d: `M${p1.x},${p1.y} Q${cpx},${cpy} ${p2.x},${p2.y}`, mx: lmx, my: lmy };
    }
  ...
  return (
    <svg viewBox="0 0 430 380" width="100%" style={{ overflow: 'visible' }}>
      <defs>
        <marker id="ah0" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
          <path d="M0,0 L7,3.5 L0,7 Z" fill="#3a3a3a" />
        </marker>
        {Object.keys(SC).map((s) => (
          <marker key={s} id={`ah-${s}`} markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto">
      ...
      {Object.entries(NP).map(([id, { cx, cy }]) => {
        ...
        return (
          <g key={id}>
            ...
            {isAct && (
              <circle cx={cx} cy={cy} r={NR - 5} fill="none" stroke={sc.col} strokeWidth=".5" opacity=".35" />
            )}
